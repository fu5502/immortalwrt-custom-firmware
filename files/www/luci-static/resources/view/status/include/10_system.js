...
var callFirmwareVersion = rpc.declare({ object: 'luci', method: 'getFirmwareVersion' });
...
return baseclass.extend({
	...
	load: function() {
		return Promise.all([
			...
			uci.load('system'),
			L.resolveDefault(callFirmwareVersion(), { version: '' })
		]);
	},
	...
	render: function(data) {
		...
		var fwVersionStr = customVersion || ((L.isObject(boardinfo.release) ? boardinfo.release.description + ' / ' : '') + (luciversion || ''));
		...
		var fields = [
			...
			_('Firmware Version'), fwVersionStr,
			_('Kernel Version'), boardinfo.kernel,
			_('Local Time'), datestr,
			_('Uptime'), systeminfo.uptime ? '%t'.format(systeminfo.uptime) : null,
			_('Load Average'), Array.isArray(systeminfo.load) ? '%.2f, %.2f, %.2f'.format(systeminfo.load[0] / 65535.0, systeminfo.load[1] / 65535.0, systeminfo.load[2] / 65535.0) : null,
